...
Removing time lags from the EC raw data
time lags were detected using the PWB method as implemented in diive
diive has a TUI for this processing that can be started with

```powershell
uv run diive-tlag-pwb-detect-remove-tui
```

diive rotates the wind, detects the PWB lag, removes the lag from the raw data and then stores lag-removed 30MIN files
