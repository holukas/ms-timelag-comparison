...
- `*-2R` — covariance maximization, 10 s window, **default fallback** lag;
  also exports rotated time series (`DEFAULT`).
- `*-3` — **constant lag** (earlier 2024 runs).
- `*-4` — **PWB**: detect/remove the lag from raw data with diive's
  `diive-tlag-pwb-*` CLIs, then process fluxes with EC maximization disabled
  (see `docs/processing-steps.md`).
